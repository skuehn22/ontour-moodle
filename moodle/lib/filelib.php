...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $offline = false, $embed = false) {
    ...
    // ========================================================================================================================
    }else if ($component === 'assignsubmission_file' && ($_GET['contextid'] == 2522 || $_GET['contextid'] == 1575  || $_GET['contextid'] == 2968)) {

        if ($filearea === 'submission_files') {



           // $fileurl = moodle_url::make_file_url('/pluginfile.php', '/2968/assignsubmission_file/submission_files/20230221_100043.jpg');
         //   html_writer::empty_tag('img', array('src' => $fileurl));

            $contextid = $_GET['contextid'];
            $itemid = $_GET['itemid'];
            $file = $_GET['file'];

            $file = $fs->get_file($contextid, 'assignsubmission_file', 'submission_files', $itemid, '/', $file);
            send_stored_file($file, 60*60, 0, 0, $sendfileoptions);

          //   $file = $fs->get_file('2967', 'assignsubmission_file', 'submission_files', '1753', '/', 'cdv_photo_1666866474.MOV');
          //   send_stored_file($file, 60*60, 0, 0, $sendfileoptions);
            ...
            $fileinfo = array(
                ...
                'itemid' => 2351,               // usually = ID of row in table
                'contextid' => 2968, // ID of context
                'filepath' => '/',           // any path beginning and ending in /
                'filename' => '20230221_100043.jpg'); // any filename
            ...
            if ($file) {
               ...
               echo $contents = $file->get_content();
