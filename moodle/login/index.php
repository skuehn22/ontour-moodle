<?php
	header("Location: https://reisen.ontour.org/zugangscode/");

/*

if(!isset($_GET['admin']) || ){
	header("Location: https://ontour.org/my-account/");
}
...
$sql = "SELECT * FROM {booking_data3} WHERE user_id = $USER->id";
$data = $DB->get_record_sql($sql);

if($USER->username == 'aufgaben#_2911' || $USER->username == 'julia_bauer574#'){
    $SESSION->wantsurl='https://reisen.ontour.org/mod/dashboard/view.php?id=192';
}else{

    if($data->product == null){
        $SESSION->wantsurl='https://reisen.ontour.org/course/view.php?id=8';
    }else{
        $SESSION->wantsurl='https://reisen.ontour.org/course/view.php?id='.$data->product;
    }
}
