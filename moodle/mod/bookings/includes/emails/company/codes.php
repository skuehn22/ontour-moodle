...
if($booking->operators_id == 1 || $booking->operators_id == 9){
    $email_subject = "ZCODESS Videoprojekt Berlin - Buchungs-Nr. ".$b_id." ".$sub_kw;
}else{

    if($booking->newsletter == 9){
        $email_subject = "ZCODESS Videoprojekt Berlin - ".$booking->ext_booking_id. " - ".$b_id." ".$sub_kw;
    }else{
        $email_subject = $op->name."ZCODESS  Videoprojekt Berlin - Nr. Veranstalter. ".$booking->ext_booking_id." ".$sub_kw;
    }
