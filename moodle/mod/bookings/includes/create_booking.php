...
if($_SERVER['HTTP_HOST'] == "www.projektreise.sk"){
    ...
    $params = array('id' => $_POST['city_product']);
}
...
if($opid != 1 && $opid != 9){
    if($operator_check_id){
        header('Location: ../view.php?id=172&status=error&error="Buchungsnummer Veranstanlter bereits vorhanden"');
        exit();
    }
}
...
foreach ($_POST['school_classes'] as $key => $class){
    ...
    createWordpressProductVariantsBooking($v_code, $class ,$ontour_bid, $key);
    ...
    for ($i = 1; $i <= 6; $i++) {
        ...
        if($i != 6){
            ...
            $token = copyUserToApp($id, $obj, '1',$_POST['city_product']);
            ...
            $content = createPDF($i, $class, $obj->username, $ontour_bid, $path_code);
            $pdf_pages[] = $content;
        }else{
            $token = copyUserToApp($id, $obj, '0', $_POST['city_product']);
            ...
            $pdf_path = "/finishing/".$pdf_name;
            $content = createPDFTeacher($i, $class, $obj->username, $ontour_bid, $path_code);
    ...
    if($operator->id == 1){
        createInvoce($ontour_bid, "group", $class, "username", "order");
    }
...
function createInvoce($ontour_bid, $group, $class, $username, $order){

    $pdf_date = date('d-m-Y');
    $pdf_name = $ontour_bid."-Rechnung.pdf";
    $pdf_path = "/invoices/".$pdf_name;
    $content = createInvoicePDF($group, $class, $username, $order);
    $pdf_pages[] = $content;
...
function createWordpressProductVariantsBooking($v_code, $class_name, $ontour_bid, $key){
...
function createVcodeUser($v_code, $class, $b_id){
    ...
    if(empty($check)){
        ...
        copyUserToApp($user, $obj, '0', $_POST['city_product']);
...
function createPDFTeacher2($group, $class, $username, $order){
...
function createInvoicePDF($group, $class, $username, $order){
...
function pdfs(){
    ...
    global $CFG;
        ...
        $class = "10a";
        $i = 5;
        $username = "g5_536_4891_10a";

        $pdf_date = date('d-m-Y');
        $pdf_name = $ontour_bid."-".$class."-Gruppe".$i."-App-Anmeldedaten.pdf";
        $pdf_path = "/finishing/".$pdf_name;
        $content = createPDF($i, $class, $username, $ontour_bid);
        $pdf_pages[] = $content;
        $school_class_name = "Klasse_1";
...
function copyUserToApp($id, $obj, $role, $product){
    ...
    $sql = "INSERT INTO users (name, email, password, moodle_uid, login_token, role, course) VALUES ('$obj->username', '$obj->email', '$obj->password', '$id', '$token', '$role', '$product')";
...
if($_SERVER['HTTP_HOST'] == "www.projektreise.sk"){
    header('Location: ../view.php?id=15&status=success&b_id='.$ontour_bid);
}else{
    header('Location: ../view.php?id=172&status=success&b_id='.$ontour_bid);
}
